...
export const WithdrawModal: React.FC<WithdrawModalProps> = ({
	...
	const initialAmount = initialOperation?.amount ?? 0;
	const grossProfit = currentBalance - initialAmount;
	...
	const handleWithdraw = () => {
		...
		onWithdraw(grossProfit, address);
		setSubmitted(true);
		setTimeout(() => {
			setSubmitted(false);
			setAddress("");
				...
				<>
					...
					<div className="bg-teal-50 dark:bg-teal-900/20 px-8 py-8 border-b border-teal-100 dark:border-teal-800">
						...
						<p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
							Review your profit breakdown before confirming the withdrawal.
						</p>
					</div>
					...
					<div className="px-8 py-6">
						...
						<div className="space-y-3">
							<div className="flex justify-between items-center py-2">
								<span className="text-sm text-slate-500">{initial.label}</span>
								{initial.value && (
							...
							<div className="flex justify-between items-center py-2">
								<div>
									...
									<p className="text-xs text-slate-400 mt-0.5">
										Sent to platform wallet automatically
									</p>
								</div>
								<span className="text-sm font-mono font-medium text-slate-900 dark:text-white">
									-
							...
							<div className="bg-teal-50 dark:bg-teal-900/20 border-2 border-teal-500 rounded-lg p-5">
								<div className="flex justify-between items-center mb-1">
									<span className="text-sm font-semibold text-slate-900 dark:text-white">
										Net Amount to You (93%)
									</span>
								</div>
								<p className="text-3xl font-mono font-bold text-teal-600 dark:text-teal-400">
									{netToClient.toLocaleString("en-US", {
										minimumFractionDigits: 2,
									})}{" "}
									USDT
								</p>
								<p className="text-xs text-slate-500 mt-1">
									This is the amount that will be sent to your wallet.
								</p>
							</div>
						</div>
						...
						<div className="mt-6 space-y-3">
							...
							<label className="flex items-start gap-3 cursor-pointer">
								...
								/>
								<span className="text-sm text-slate-700 dark:text-slate-300">
									I understand the 7% platform fee will be deducted
									automatically.
								</span>
							</label>
						</div>
					...
					<div className="px-8 pb-8">
						...
						<p className="mt-3 text-xs text-center text-slate-400">
							The 7% fee ({platformFee.toFixed(2)} USDT) will be sent to the
							platform wallet automatically.
							<br />
							The remaining 93% ({netToClient.toFixed(2)} USDT) will be sent to
							your address.
						</p>
					</div>
				</>
