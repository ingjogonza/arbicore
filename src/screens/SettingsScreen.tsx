...
import { 
  Settings, Link2, BarChart3, FileText, AlertTriangle, 
  CheckCircle, Download, Trash2, Info
} from 'lucide-react';
    ...
    <DashboardLayout>
        ...
        <div className="lg:col-span-3">
          {activeSection === 'legal' && (
            <div className="space-y-4">
              <Card className="p-4 md:p-6">
                <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-1">Legal Documents</h3>
                <p className="text-sm text-slate-500 mb-6">Review and accept all required legal agreements to use the platform.</p>

                <div className="space-y-4">
                  {legalDocs.map(doc => (
                    <div key={doc.id} className="flex items-start gap-4 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg">
                ...
                <div className="mt-6 p-4 bg-teal-50 dark:bg-teal-900/20 rounded-lg flex items-center gap-3">
                  <CheckCircle size={20} className={`${allAccepted ? 'text-teal-600' : 'text-slate-400'}`} />
                  <span className={`text-sm font-medium ${allAccepted ? 'text-teal-900 dark:text-teal-200' : 'text-slate-500'}`}>
                    {allAccepted ? 'All documents accepted' : `${legalDocs.filter(d => d.accepted).length} of ${legalDocs.length} accepted`}
                  </span>
                </div>
              </Card>
            </div>
          ...
          {activeSection === 'danger' && (
            <Card className="p-4 md:p-6 border-red-200 dark:border-red-800">
              ...
              <p className="text-sm text-slate-500 mb-6">These actions are irreversible. Please proceed with caution.</p>

              <div className="space-y-4">
                <div className="flex items-center justify-between p-4 border border-slate-200 dark:border-slate-700 rounded-lg">
                  <div>
                    <h4 className="font-medium text-slate-900 dark:text-white">Disconnect Binance API</h4>
                    <p className="text-sm text-slate-500 mt-0.5">Remove API access and stop all automated trading.</p>
                  </div>
                ...
                <div className="flex items-center justify-between p-4 border border-slate-200 dark:border-slate-700 rounded-lg">
                  <div>
                    ...
                    <p className="text-sm text-slate-500 mt-0.5">Permanently delete your account and all associated data.</p>
                  </div>
          ...
          {activeSection === 'account' && (
            <Card className="p-4 md:p-6">
              ...
              <div className="space-y-4">
                <Input label="Full Name" value="Alex Rivera" onChange={() => {}} />
                <Input label="Email" value="alex@example.com" onChange={() => {}} />
          ...
          {activeSection === 'api' && (
            <Card className="p-4 md:p-6">
              <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-4">API Management</h3>
              <Alert variant="info" icon={<Info size={16} />}>
                Your API keys are encrypted at rest using AES-256.
              </Alert>
              <div className="mt-4 p-4 bg-slate-50 dark:bg-slate-800 rounded-lg">
                <p className="text-sm font-medium text-slate-900 dark:text-white">Connected API Key</p>
                <p className="text-sm font-mono text-slate-500 mt-1">••••••••••••••••</p>
              </div>
            </Card>
          )}
          ...
          {activeSection === 'risk' && (
            <Card className="p-4 md:p-6">
              <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-4">Risk Configuration</h3>
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Risk Profile</label>
                ...
                <div>
                  ...
                  <input type="number" defaultValue={10} className="w-full px-4 py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-md text-sm" />
                </div>
      ...
      <Modal isOpen={showDisconnectModal} onClose={() => setShowDisconnectModal(false)}>
        <div className="p-8">
          ...
          <h3 className="text-xl font-bold text-center text-slate-900 dark:text-white mb-2">Disconnect API?</h3>
          <p className="text-sm text-center text-slate-500 mb-6">
            This will stop all automated trading and remove API access. You can reconnect anytime.
          </p>
          <div className="flex gap-3">
            <Button variant="secondary" onClick={() => setShowDisconnectModal(false)} className="flex-1">Cancel</Button>
            <Button variant="danger" onClick={() => { disconnectApi(); setShowDisconnectModal(false); navigate('/connect'); }} className="flex-1">Disconnect</Button>
          </div>
        </div>
      </Modal>
      ...
      <Modal isOpen={showDeleteModal} onClose={() => setShowDeleteModal(false)}>
        <div className="p-8">
          ...
          <div className="flex gap-3">
            ...
            <Button variant="danger" onClick={() => setShowDeleteModal(false)} className="flex-1">Delete Account</Button>
          </div>
        </div>
      </Modal>
    </DashboardLayout>
