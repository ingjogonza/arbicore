...
export const DashboardScreen: React.FC = () => {
	...
	const showBanner =
		!onboarding.loading && missingSteps.length > 0 && !dismissed;
	...
	return (
		<DashboardLayout>
			<div className="space-y-4 md:space-y-6">
				...
				{/* All set banner */}
				{!onboarding.loading && missingSteps.length === 0 && (
				...
				<div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
						...
						<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-4 md:mb-6">
							...
							<div className="flex gap-1">
								{["1D", "1W", "1M", "3M", "ALL"].map((period) => (
									<button
										key={period}
										className="px-3 py-1 text-xs font-medium rounded-md bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors"
									>
										{period}
									</button>
								))}
					...
					<Card className="p-4 md:p-6">
						...
						<div className="flex items-center gap-3 mb-6">
							<div
								className={`w-3 h-3 rounded-full ${account.botStatus === "active" ? "bg-emerald-500 animate-pulse" : account.botStatus === "paused" ? "bg-amber-500" : "bg-red-500"}`}
							/>
							<Badge
								variant={
							...
							>
								{account.botStatus.charAt(0).toUpperCase() +
									account.botStatus.slice(1)}
							</Badge>
						</div>
						...
						<div className="space-y-2">
							<Button
								variant={
									account.botStatus === "active" ? "secondary" : "primary"
								}
								onClick={toggleBot}
								className="w-full"
							>
								...
								{trades.map((trade) => (
									...
									>
										...
										<td className="px-3 md:px-6 py-3">
											<Badge
												variant={trade.type === "buy" ? "info" : "warning"}
											>
										...
										<td
											className={`px-3 md:px-6 py-3 text-xs md:text-sm text-right font-mono font-medium ${trade.pnl > 0 ? "text-emerald-600" : trade.pnl < 0 ? "text-red-600" : "text-slate-500"}`}
										>
											{trade.pnl > 0 ? "+" : ""}
											{trade.pnl.toFixed(2)}
										</td>
